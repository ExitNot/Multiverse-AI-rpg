...
The player's primary objective is to find and absorb a {time_anomaly} - a mysterious object or entity that exhibits strange gravitational properties.
Always print variables as simpe strings without brces"""

DEFINITIONS = """Game Variables:
...
2. {mission}:
   ...
   - Ends when player successfully absorbs the {time_anomaly}
...
2. One {location} must contain the {time_anomaly}
...
2. For {npc} interactions:
   ...
   - Use their {npc_knowledge} appropriately
...
Always maintain game consistency and track player progress toward finding the {time_anomaly}. Use conversation history and rely on it with proper continuing of the story"""
